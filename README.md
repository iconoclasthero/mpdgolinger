...
Classic rock stations have long had gimmicks like "Rock Blocks," "Double Shots," "Twofer Tuesdays," "Workfoce blocks," etc., where multiple songs from the same artist or around a common theme were played consecutively. That's essentially what this does. Set your limit=_n_ (default _n_=4) and mpd playback will _linger_ on the playlist for _n_ songs before advancing on random to another block. This is compatible with consume and repeat mpd playback modes. And if you're really digging, say, a particular Dead bootleg and want to hear more than _n_ songs for a block, you can temporarily override the limit for a block.
